        ...
        EmptyView()
            .alert("Correcto", isPresented: $showAlertCorrect) {
                Button("Ok",role: .cancel) {
                    DataController().addProduct(title: textFieldTitle, imageURL: textFieldImage, category: textFieldCategory, rating: Int16(textFieldRating) ?? 0, description: textFieldDescription, price: Double(textFieldPrice) ?? 0.0, context: moc)
                    resetFields()
                }
            } message: {
